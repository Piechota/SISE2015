...
Decision LewandowskiFuzzy::ProcessAI(const std::vector<NodeInfo> graphInfo, const Pawn* const myPawn)
{
    ...
    output->addTerm(new Triangle("MOVE", 0.195, 0.8, 1.0));
    ...
    double * neighbours = new double[neighboursCount];
    ...
    for (int i = 0; i < neighboursCount; ++i)
    {
        ...
        *std::min_element(
           tmp.begin(),
            tmp.end(),
            [](DistanceToPlayer d1, DistanceToPlayer d2){return d1.distance < d2.distance; });
    ...
    else
    {
        ...
        int targetID =
            graphInfo.at(myNeighboursID.at(moveIndx.at(rand() % moveIndx.size()))).myId;
